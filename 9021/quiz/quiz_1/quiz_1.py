...
def describe_automaton(transitions):
    ...
    for key in transitions :
        print('When in state "{a}" and processing "{b}", automaton\'s state becomes "{c}".'.format(a=key[0],b=key[1],c=transitions[key]))
...
('state_2', 0): 'state_1', ('state_2', 1): 'state_2'}
    ...
    transitions = {}
    for n in transitions_as_list :
        str_t0, str_t1 = n.split (':')
        str_a0, str_a1 = str_t0.split(',')
        key = (str_a0,int(str_a1))
        transitions[key] = str_t1
...
def accepts(transitions, word, initial_state, accept_state):
    ...
    for key in transitions:
        list_t.append(key[1])
    for n in word :
        if int(n) not in list_t:
            flag=1
        else:
            key = (cur_state,int(n))
            if key in transitions :
                cur_state =transitions[key]
            else :
                flag = 1
    if((cur_state == accept_state)&(flag==0)):
        return(True)
    else :
        return(False)
